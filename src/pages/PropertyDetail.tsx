...
export default function PropertyDetail() {
  ...
  const ADMIN_EMAIL = "vohuutrungkn@gmail.com";
  ...
  return (
    <>
      <div className="container py-4 lg:py-8 space-y-6 lg:space-y-8 pb-24 md:pb-10">
        ...
        <div className="flex flex-col gap-3">
          ...
          {isAdmin && (
            <Link
              ...
              className="inline-flex items-center gap-2 bg-secondary text-secondary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:bg-secondary/80 transition-colors w-fit"
            >
        ...
        <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-card rounded-lg p-4 sm:p-5 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-muted-foreground">
                ...
                {property.floors && <DetailRow label="Số tầng" value={property.floors} />}
                {property.bedrooms && <DetailRow label="Số phòng ngủ" value={property.bedrooms} />}
                {property.bathrooms && <DetailRow label="Số phòng tắm" value={property.bathrooms} />}
                {property.legal_documents && <DetailRow label="Giấy tờ pháp lý" value={property.legal_documents} />}
                {property.interior && <DetailRow label="Nội thất" value={property.interior} />}
                {property.house_direction && <DetailRow label="Hướng nhà" value={property.house_direction} />}
                {property.balcony_direction && <DetailRow label="Hướng ban công" value={property.balcony_direction} />}
                {property.access_road && <DetailRow label="Đường vào" value={`${property.access_road} m`} />}
                {property.frontage && <DetailRow label="Mặt tiền" value={`${property.frontage} m`} />}
              </div>
            </section>
          ...
          <div className="space-y-4 lg:space-y-6 lg:sticky lg:top-24 h-fit">
            <section className="bg-card rounded-xl p-5 space-y-4 shadow-sm border">
              {/* Agent Info */}
              ...
              <div className="flex flex-col gap-3">
                <a
                  href="tel:0996668800"
                  className="flex items-center gap-2 bg-destructive hover:bg-destructive/90 text-destructive-foreground px-4 py-3 rounded-lg shadow-sm justify-center font-semibold text-sm transition-colors"
                >
                ...
                <a
                  ...
                  className="w-full inline-flex items-center justify-center gap-2 rounded-lg border-2 border-primary text-primary py-3 text-sm font-semibold bg-background hover:bg-primary/5 transition-colors"
                >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background border-t shadow-lg z-50 md:hidden p-2">
        <div className="flex items-center gap-2">
...
function AgentAvatar({ size = "normal" }: { size?: "small" | "normal" }) {
  const [imageError, setImageError] = useState(false);
  const sizeClasses = size === "small" ? "w-10 h-10" : "w-14 h-14";
  const textSize = size === "small" ? "text-sm" : "text-lg";

  if (imageError) {
    return (
      <div className={`${sizeClasses} rounded-full bg-primary flex items-center justify-center`}>
        <span className={`${textSize} font-semibold text-primary-foreground`}>VT</span>
      </div>
    );
  }

  return (
    <div className={`${sizeClasses} rounded-full overflow-hidden bg-muted`}>
      <img
        src="/agent-trung.png"
        alt="Võ Hữu Trung"
